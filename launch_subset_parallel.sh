...
for job in "${JOBS[@]}"; do
  ...
  sbatch --partition=mi210 --gres=gpu:1 --job-name=gen_${part} \
    --output=logs/gen_exp_${part}_%j.log --cpus-per-task=4 --mem=16G \
    --time=1:00:00 \
    --wrap="cd /home/jpalominos/VT_Model_for_LightCurves_Classification && source venv/bin/activate && python scripts/generate_subset_one.py $fname $label"
  echo "Lanzado: $part"
done
